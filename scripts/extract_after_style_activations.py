"""Extract teacher-forced activations for AFTER-style factual-text pairs."""
...
REQUIRED_FIELDS = ("id", "image_id", "question", "factual_answer", "counterfactual_answer", "hallucination_type", "subtype")


def parse_args() -> argparse.Namespace:
    """Parse CLI arguments for AFTER-style activation extraction."""
    ...
    parser.add_argument("--pair-file", required=True, help="AFTER-style pair JSONL.")
    ...
    parser.add_argument("--position-mode", choices=["answer_last_token"], default="answer_last_token")
...
def require_torch() -> Any:
    ...
    try:
        ...
        raise RuntimeError("extract_after_style_activations requires torch.") from exc
...
def validate_row(row: Mapping[str, Any], row_index: int) -> None:
    """Validate one AFTER-style pair row."""
...
def metadata_row(
    ...
    """Build one metadata row for an AFTER-style activation."""
    ...
    return {
        ...
        "counterfactual_fact": str(row.get("counterfactual_fact", "")),
        "source": str(row.get("source", "after_style_v1")),
...
def main() -> int:
    """Extract and save AFTER-style activations."""

    args = parse_args()
    try:
        torch = require_torch()
        if int(args.batch_size) != 1:
            raise ValueError("AFTER-style v1 extraction currently supports --batch-size 1 only")
        ...
        for row_index, row in enumerate(rows):
            ...
            result = mock_result(adapter, row) if args.adapter == "mock" else adapter.encode_pair(
                image_path=image_path,
                question=str(row["question"]),
            ...
            metadata_rows.append(metadata_row(row_index, row, image_path, args.split, result["meta"], args.adapter, model_id))
            if int(args.progress_every) > 0 and (row_index + 1) % int(args.progress_every) == 0:
                print(f"[extract_after_style] processed {row_index + 1}/{len(rows)} rows")
        ...
        manifest = {
            ...
            "notes": [
                ...
                "z_factual - z_counterfactual is the AFTER-style direction source",
            ],
        }
        ...
        return 1
    print(f"Wrote AFTER-style activations to {output_path}")
