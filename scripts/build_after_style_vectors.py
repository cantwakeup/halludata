"""Build cat/attr/rel expert vectors from AFTER-style activations."""
...
def parse_args() -> argparse.Namespace:
    """Parse CLI arguments for AFTER-style vector construction."""

    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("--activation-cache", required=True)
    parser.add_argument("--metadata", required=True)
    parser.add_argument("--output", default="data/outputs_after_style_v1/steering/after_style_expert_vectors.pt")
    parser.add_argument("--stats-output", default="data/outputs_after_style_v1/steering/after_style_expert_vectors.stats.json")
...
def require_torch() -> Any:
    ...
    try:
        ...
        raise RuntimeError("build_after_style_vectors requires torch.") from exc
...
def main() -> int:
    """Build and save AFTER-style expert vectors."""
    ...
    try:
        ...
        if (output_path.exists() or stats_path.exists()) and not args.overwrite:
            raise FileExistsError("Output exists. Pass --overwrite to replace after_style_v1 vector outputs.")
        ...
        if len(metadata_rows) != int(z_factual.shape[0]):
        ...
        stats = {
            ...
            "warnings": warnings,
            "notes": [
                "vectors are factual minus counterfactual activation means",
        ...
        return 1
    print(f"Wrote AFTER-style expert vectors to {output_path}")
